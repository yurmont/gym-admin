...
export async function registerPayment(
  ...
  await ctx.db.query(
    `insert into public.payments (id,tenant_id,member_id,membership_id,code,concept,amount,discount,total,method,reference,status,paid_at,user_id)
  ...
  if (ms) {
    ...
    await ctx.db.query(
      "update public.memberships set paid_amount=$1,status=$2,updated_at=now() where id=$3 and tenant_id=$4",
      [money(paid), active ? "activa" : "pendiente_pago", ms.id, ctx.tenant],
    );
    await ctx.db.query(
      "update public.members set status=$1,updated_at=now() where id=$2 and tenant_id=$3 and status in ('activo','inactivo','moroso')",
...
export async function voidPayment(
  ...
  const payment = await required(
    ctx.db,
    "select * from public.payments where id=$1 and tenant_id=$2 and status='pagado' for update",
    [paymentId, ctx.tenant],
    "Pago no disponible para anulación",
  );
  await ctx.db.query(
    "update public.payments set status='anulado',voided_at=now(),voided_by=$1,updated_at=now() where id=$2 and tenant_id=$3",
    [ctx.actor, paymentId, ctx.tenant],
  );
  if (payment.membership_id) {
    const ms = await membership(ctx, String(payment.membership_id));
    await ctx.db.query(
      "update public.memberships set paid_amount=$1,status='pendiente_pago',updated_at=now() where id=$2 and tenant_id=$3",
      [
    ...
    await ctx.db.query(
      "update public.members set status='moroso',updated_at=now() where id=$1 and tenant_id=$2 and status<>'baja'",
